...
title: "Config конфигурация - CyberGo JSON | Справочник API"
description: "Config CyberGo JSON: DefaultConfig по умолчанию, SecurityConfig безопасная, PrettyConfig форматирование с кэшем и лимитами — настройка поведения JSON в Go."
sidebar_position: 4
---
...
::: warning Статус резервирования полей расширения
Поля `CustomEncoder`, `CustomTypeEncoders` и `CustomValidators` объявлены в текущей версии, но **ещё не подключены к конвейеру кодирования/операций**. Их установка не даёт эффекта — они зарезервированы для будущих версий. Доступный в настоящее время способ настройки кодирования — реализация интерфейса `json.Marshaler` или `encoding.TextMarshaler` (см. [Пользовательский кодировщик](../extensions/custom-encoder)); доступный метод валидации — `ValidateSchema` (см. [Валидатор](../extensions/validator)).
:::
