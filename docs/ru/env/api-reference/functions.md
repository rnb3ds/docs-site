...
description: "Справочник пакетных функций CyberGo env: Load, GetString, GetInt, GetBool, GetDuration, GetSlice, GetSecure и ParseInto на глобальном загрузчике."
sidebar_position: 2
---
...
## Дополнение типизированного чтения

### GetUint64 / GetFloat64

Помимо распространённых типов, функции уровня пакета и методы Loader дают чтение беззнаковых целых и чисел с плавающей точкой:

<!-- check-code: skip -->
```go
limit := env.GetUint64("RATE_LIMIT", 1000)     // uint64
rate := env.GetFloat64("SAMPLE_RATE", 0.5)      // float64
u := loader.GetUint64("RATE_LIMIT", 1000)       // та же сигнатура у Loader
f := loader.GetFloat64("SAMPLE_RATE", 0.5)
```

### Свободный разбор логических значений

`GetBool` без учёта регистра принимает следующие написания:

| Истина | Ложь |
|--------|------|
| `true`, `yes`, `on`, `1`, `enabled` | `false`, `no`, `off`, `0`, `disabled` |

Пустая строка разбирается как `false`; любое другое значение порождает `ValidationError` и **возвращает значение по умолчанию** (также записывается событие аудита).

### Сбои разбора и значения по умолчанию

Все функции `Get*` возвращают значение по умолчанию (или нулевое), когда ключ **отсутствует** или значение **не удалось разобрать** — они никогда не паникуют и не возвращают error. Сбои разбора записываются в аудит как `parse failed`, что упрощает диагностику грязной конфигурации.

### Две семантики сбоев в GetSliceFrom

В режиме экземпляра `GetSliceFrom[T](loader, key)` собирает элементы из индексных ключей (`KEY_0`, `KEY_1`, ...) с откатом к значению через запятую. Сбои разбора различаются по источнику:

| Источник | При сбое разбора элемента |
|----------|---------------------------|
| Индексные ключи (`KEY_0`...) | Элемент **пропускается** (после записи аудита); остальные сохраняются |
| Откат к запятой | **Весь список проваливается**; возвращается значение по умолчанию (или nil) |

Ограничение обобщённого типа элемента: `string`, `int`, `int64`, `uint`, `uint64`, `bool`, `float64`, `time.Duration`. Предел длины среза — 10000 (защитный лимит, превышение аудируется).

### LoadWithConfig принудительно включает AutoApply

И `Load()`, и `LoadWithConfig()` **принудительно** задают `AutoApply=true` независимо от переданной конфигурации — функциям уровня пакета нужны переменные в окружении процесса. Для ручного управления моментом применения используйте `New()` (режим экземпляра).
