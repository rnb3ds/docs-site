...
description: "Справочник SecureValue CyberGo env: NewSecureValue, блокировка памяти mlock, Reveal, Release и IsSensitiveKey для хранения паролей и токенов."
sidebar_position: 5
---
...
## Защищённые от утечки интерфейсы и детали жизненного цикла

### Безопасные интерфейсы строкового представления

`SecureValue` реализует набор интерфейсов «безопасной стрингизации», чтобы секреты **никогда** не утекали через распространённые пути форматирования:

| Интерфейс | Поведение |
|-----------|-----------|
| `fmt.Stringer` (`String()`) | Возвращает маскированную форму (как `Masked()`); `fmt.Printf("%v")`, `log.Println` безопасны |
| `json.Marshaler` (`MarshalJSON()`) | JSON-сериализация выдаёт маскированную строку; открытый текст никогда не попадает в JSON |
| `encoding.TextMarshaler` (`MarshalText()`) | Текстовые кодировщики (XML, text/template, структурные логгеры) выдают маску |

Единственный явный путь к открытому тексту — `Reveal()` (или копия `Bytes()`).

### Close против Release

| Метод | Обнуляет данные | Возвращает в пул | Применение |
|-------|:---:|:---:|------|
| `Close()` | Да | Нет | Обычное разовое использование |
| `Release()` | Да | Да | Высокочастотные циклы создания/уничтожения (повторное использование снижает аллокации) |

Оба безопасно вызывать многократно без побочных эффектов. Брошенные незакрытые объекты тоже защищены: GC-финализатор обнуляет данные, когда объект становится недостижим (`runtime.SetFinalizer` устанавливается один раз при создании; циклы пула не повторяют эту цену).

### Формат вывода Masked

Маскированное представление несёт статус блокировки, позволяя проверять защиту из логов:

```text
[SECURE:24 bytes]            // блокировка памяти не включена
[SECURE:24 bytes locked]     // заблокировано
[SECURE:24 bytes lock-failed] // сбой блокировки (сверьте с MemoryLockError)
[CLOSED]                      // закрыто
[NIL]                         // nil-приёмник
```

### Запросы состояния блокировки

- `IsMemoryLocked()`: заблокирована ли память значения сейчас
- `MemoryLockError()`: ошибка последней попытки блокировки (nil при успехе или если не выполнялась); первый инструмент диагностики прав в строгом режиме

### GetSecure возвращает защитную копию

`*SecureValue`, возвращаемый `Loader.GetSecure`, — **независимая копия** хранимого значения — её жизненный цикл отделён от Loader. Вызывающий владеет `Close`/`Release`; внутренний оригинал Loader не затрагивается.

### ClearBytes

Копию, возвращённую `Bytes()`, следует явно обнулять после использования:
...
func main() {
	...
	data := sv.Bytes()
	defer env.ClearBytes(data) // обнулить после использования, без остатка
