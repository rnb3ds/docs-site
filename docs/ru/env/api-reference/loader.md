...
description: "Справочник API Loader CyberGo env: мультиформатная загрузка LoadFiles, типобезопасное чтение, Set/Delete и валидация — все методы потокобезопасны."
sidebar_position: 3
---
...
## Жизненный цикл и семантика окружения процесса

### Close не откатывает os.Environ

`Close()` обнуляет только **в памяти** копии переменных и **не** снимает переменные, ранее записанные в окружение процесса через `Apply`, `AutoApply` или `Set` (при включённом `AutoApply`). Чтобы удалить их до закрытия, вызывайте `Delete` по каждому ключу:

<!-- check-code: skip -->
```go
loader.Delete("API_KEY") // снимает из окружения, только если этот loader его установил
loader.Close()
```

### Посключевое отслеживание владения в Delete

Loader записывает, какие ключи **он сам** записал в окружение процесса (`appliedKeys`). `Delete` снимает только эти ключи и **никогда не трогает** переменные процесса, которые он не устанавливал (`HOME`, `TERM`, ...). Поэтому несколько Loader не мешают друг другу.

### Apply различает «не задано» и «явно пусто»

`Apply` использует `LookupEnv` для обнаружения существующих значений: переменная, явно установленная в пустую строку, считается «присутствующей» и тоже пропускается при `OverwriteExisting=false` — она не ошибочно принимается за незаданную и не перезаписывается.

### Методы состояния и метаданных

| Метод | Описание |
|-------|----------|
| `Config()` | Снимок конфигурации только для чтения; изменение возвращённого значения не влияет на работающий loader |
| `IsClosed()` | Закрыт ли loader (nil-приёмник возвращает true) |
| `IsApplied()` | Применены ли переменные к os.Environ |
| `LoadTime()` | Время последней загрузки (нулевое значение, если загрузок не было) |

### Детали разрешения имён ключей на пути чтения

- **Сначала точное совпадение**: поиск по ключу как есть, затем откат к форме в верхнем регистре
- **Точечные пути**: `database.host` разворачивается в `DATABASE_HOST` (уплощённые ключи JSON/YAML); индексный путь `service.cors.origins.0` откатывается к значению через запятую, если индексный ключ не найден
- **Без trim**: `Lookup` возвращает сохранённое значение как есть — значения `.env` обрезаны при разборе, значения JSON/YAML могут сохранять окружающие пробелы
- **Быстрый путь без блокировок**: одиночные чтения (`Lookup`/`GetSecure`) обходят блокировку чтения уровня loader через атомарный флаг закрытости — при высокой конкурентности сам счётчик читателей RWMutex становится узким местом; гонки с `Close` деградируют корректно: «прочитать значение или очищенное состояние»

### Семантика молчаливого пропуска в Set

При `OverwriteExisting=false` вызов `Set` для существующего ключа **молча пропускается и возвращает nil** (без ошибки). Чтобы замечать пропуски, следите за журналом аудита (события `skipped (no overwrite)`).

### Эшелонированная защита при загрузке файлов (TOCTOU)

Между проверкой размера `Stat()` и фактическим чтением существует теоретическое окно подмены; библиотека смягчает его двумя слоями: парсеры внутренне оборачивают поток файла в усиленный путь чтения, принуждающий лимит размера, поэтому даже файл, выросший после проверки, обрезается на жёстком потолке (100 МБ), предотвращая исчерпание памяти.
