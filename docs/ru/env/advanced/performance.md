...
title: "Оптимизация производительности - CyberGo env | тюнинг чтения"
description: "Оптимизация производительности CyberGo env: сегментированные блокировки, пул объектов sync.Pool, накладные расходы mlock и бенчмарки конкурентного чтения."
sidebar_position: 1
---
...
## Глубокий разбор внутренней конкурентности

Этот раздел объясняет оптимизации конкурентности библиотеки на уровне исходного кода для оценки и тюнинга.

### Шардированное хранилище (secureMap)

Все переменные хранятся как `SecureValue` в 8 шардах; ключи размещаются хэшем FNV-1a, и каждый шард держит собственный `sync.RWMutex` — чтение и запись разных шардов идут полностью параллельно, рассеивая конкурентность блокировок.

### Бакетированная пакетная запись

`secureMap.SetAll`/`secureMap.SetAllIfAbsent` (путь загрузки файлов) раскладывают весь пакет в одну плоскую аллокацию и блокируют **каждый шард лишь один раз** на весь пакет; существующие ключи обновляются на месте (с повторным использованием объекта `SecureValue`), избегая циклов пула и лишних аллокаций.

### Оптимизации блокировок на пути чтения

- **Чтение одного ключа без блокировок**: `Lookup`/`GetSecure` обходят блокировку чтения уровня loader через атомарный флаг закрытости — при высокой конкурентности сам счётчик читателей RWMutex является узким местом
- **`secureMap.Has` без аллокаций**: атомарно читает только флаг closed, не копируя строку значения
- **`Get` пропускает вторую блокировку**: блокировки чтения шарда уже гарантирует стабильность данных `SecureValue`, поэтому блокировка уровня объекта пропускается (она когда-то давала 23.6% атомарных операций в профилях)
- **Быстрый путь `Set`**: при `OverwriteExisting=true` и выключенном `AutoApply` достаточно блокировки чтения — параллельные `Set` не сериализуются на мьютексе loader

### Пул объектов и финализатор

`SecureValue` переиспользуется через `sync.Pool`; GC-финализатор устанавливается **один раз при первом создании** (~60% стоимости создания), и циклы пула эту цену не повторяют. Перезапись существующего ключа сбрасывает на месте, ещё больше снижая аллокации.

### Семантика приближённого снимка

`Len` возвращает за O(1) через атомарный счётчик; `Keys`/`ToMap` оценивают ёмкость по счётчику и затем обходят шарды. При параллельных записях результат может быть **слегка устаревшим приближением** (модификации во время обхода не гарантированно видны) — для точного снимка синхронизируйтесь внешне.

### Осознанность производительности аудита

С включённым аудитом путь загрузки получает накладные расходы `time.Now` и построения событий (при выключении полностью избегаются). Для высоконагруженных сценариев включайте аудит только под пресетом Production или потребляйте асинхронно через `ChannelAuditHandler`.
