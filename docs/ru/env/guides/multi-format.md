...
description: "Руководство по мультиформатной конфигурации CyberGo env: загрузка .env, JSON и YAML с автоопределением, приоритет слияния ключей и преобразование форматов."
sidebar_position: 1
sidebar_icon: "🔧"
---
...
## Детали структурированного разбора и фильтрация по префиксу

### Правила уплощения JSON/YAML

Вложенные структуры уплощаются в ключи переменных окружения при загрузке. Правила по умолчанию:

| Правило | Пример |
|---------|--------|
| Вложенные ключи соединяются подчёркиваниями | `{"database": {"host": "..."}}` → `DATABASE_HOST` |
| Элементы массивов используют `ключ_индекс` | `{"hosts": ["a", "b"]}` → `HOSTS_0`, `HOSTS_1` |
| `null` становится пустой строкой | `{"name": null}` → `NAME=""` |
| Числа и логические значения — строками | `{"port": 5432}` → `PORT="5432"` |
| Предел глубины вложенности — 10 | Управляется `JSONMaxDepth`/`YAMLMaxDepth` (1–100) |

При загрузке файлов поведение преобразования управляется `JSONConfig`/`YAMLConfig` в `Config` (например, `JSONNullAsEmpty`, `JSONNumberAsString`); пути разбора строк в `UnmarshalMap`/`UnmarshalStruct` используют встроенные правила по умолчанию и эти настройки не читают.

### Фильтрация по префиксу (Prefix)

`ComponentConfig.Prefix` ограничивает загрузку переменными с указанным префиксом (без учёта регистра) — типично для выделения своей части приложения из общего файла конфигурации:

<!-- check-code: skip -->
```go
cfg := env.DefaultConfig()
cfg.Prefix = "MYAPP_" // Загружать только ключи, начинающиеся с MYAPP_
loader, _ := env.New(cfg)
```

Фильтрация по префиксу применяется совместно с политикой `OverwriteExisting`: отфильтрованные ключи не перезаписываются и не попадают в хранилище.

### Поведение для неизвестных расширений

`DetectFormat` возвращает `FormatAuto` для неизвестных расширений, и загрузчик **откатывается к парсеру `.env`**, разбирая построчно. Поэтому файлы вроде `config.conf` или `config.txt` загружаются корректно, пока их содержимое имеет вид `KEY=value`.
