...
title: "Обработка ошибок - CyberGo env | сигнальные ошибки"
description: "Руководство по обработке ошибок CyberGo env: сопоставление 16 сигнальных ошибок errors.Is, контекст ParseError через errors.As и стратегии восстановления."
sidebar_position: 5
sidebar_icon: "🔧"
---
...
## Полная сводка типов ошибок

Все структурированные типы ошибок можно извлечь через `errors.As` с контекстом:

| Тип | Сценарий | Ключевая информация |
|-----|----------|---------------------|
| `ParseError` | Сбой разбора файла | Файл, строка, содержимое (санитизировано) |
| `ValidationError` | Сбой валидации конфигурации/ключа/значения | Поле, правило, сообщение |
| `SecurityError` | Нарушение политики безопасности (запрещённые ключи, проверка путей) | Детали нарушения |
| `FileError` | Сбой файловой операции | Путь, операция, размер/лимит |
| `ExpansionError` | Сбой развёртки переменных | `Kind` (классификация причины) |
| `JSONError` | Сбой разбора JSON | Позиционная информация |
| `YAMLError` | Сбой разбора YAML | Позиционная информация |
| `MarshalError` | Сбой маршалинга/анмаршалинга | Операция и причина |

### ExpansionErrorKind

`ExpansionError` различает два класса сбоев через поле `Kind` для точной обработки:

| Константа | Значение |
|-----------|----------|
| `ExpansionDepthKind` | Превышен предел рекурсии или обнаружен цикл |
| `ExpansionRequiredKind` | Переменная из `${VAR:?message}` не установлена |

### Дополнительные сентинельные ошибки

Помимо распространённых из раздела [сентинельных ошибок](#сентинельные-ошибки):

| Сентинель | Значение |
|-----------|----------|
| `ErrClosed` | Операции над закрытым (или nil) Loader |
| `ErrInvalidConfig` | Неверная конфигурация; `New()` оборачивает конкретную ошибку валидации |
| `ErrNotInitialized` | Функции записи в глобальном режиме до вызова `Load()` |
| `ErrAlreadyInitialized` | Повторный вызов `Load()` после инициализации загрузчика по умолчанию |
| `ErrDuplicateKey` | Зарезервировано: дубликаты ключей сейчас молча пропускаются при `OverwriteExisting=false`; возвращающих путей нет |

### Вспомогательные предикаты

`IsMarshalError(err)` через `errors.As` проверяет, является ли ошибка `*MarshalError` — ручное приведение не нужно.
