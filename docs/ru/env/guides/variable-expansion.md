...
## Область развёртки переменных (изоляция безопасности)

По умолчанию развёртка переменных разрешает ссылки в порядке «сначала файл, затем окружение процесса» — если `${VAR}` не найден в файле, поиск откатывается к окружению процесса (`os.LookupEnv`). Это соответствует традиционной семантике dotenv, но становится риском, когда файл конфигурации приходит из **не полностью доверенного источника** (загрузки пользователей, передача внешними системами): файл с `${AWS_SECRET_ACCESS_KEY}` после загрузки может «захватить» посторонние секреты процесса в значения переменных, которые затем утечкут через логирование, сериализацию или экспорт.

`ParsingConfig.ExpansionScope` управляет этим поведением:

| Значение | Поведение | Сценарий |
|----------|-----------|----------|
| `ExpansionFileThenProcess` (по умолчанию) | Сначала переменные файла, откат к окружению процесса | Доверенные файлы, традиционная семантика dotenv |
| `ExpansionFileOnly` | Видны только переменные файла; ссылки на окружение процесса разворачиваются в пустые строки | Недоверенные источники конфигурации, защита от зондирования секретов (SEC-03) |

<!-- check-code: skip -->
```go
cfg := env.DefaultConfig()
// Разрешаем ссылки только на переменные файла, блокируя чтение окружения процесса
cfg.ExpansionScope = env.ExpansionFileOnly
loader, _ := env.New(cfg)
```

::: warning SEC-03
`ExpansionFileOnly` — защитный переключатель, не позволяющий файлам конфигурации «собирать урожай» секретов процесса. Если приложение загружает предоставленные извне файлы конфигурации, держите эту область включённой всегда.
:::

Примечание: `ExpansionScope` — перечисление, а не логическое поле. Даже если это единственное установленное поле в `Config`, `IsZero()` корректно распознает конфигурацию как инициализированную и она не будет молча заменена значениями по умолчанию.
