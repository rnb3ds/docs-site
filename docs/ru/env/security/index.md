...
description: "Обзор архитектуры безопасности CyberGo env: блокировка памяти и автоочистка SecureValue, валидация ключей и значений, IsSensitiveKey и журнал аудита."
sidebar_position: 1
sidebar_icon: "🛡️"
---
...
## Изоляция области развёртки

Развёртка переменных по умолчанию разрешает ссылки «сначала файл, затем окружение процесса». Для недоверенных источников конфигурации `ExpansionScope: ExpansionFileOnly` ограничивает ссылки пределами файла, блокируя зондирование и захват процессных секретов (учётные данные облаков, токены и т.д.) файлом конфигурации (SEC-03). Способ настройки и сценарии атак описаны в [Развёртка переменных · Область развёртки](/ru/env/guides/variable-expansion).
