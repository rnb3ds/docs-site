...
description: "Руководство по системе хуков CyberGo DD: 6 событий жизненного цикла (BeforeLog, AfterLog, OnFilter, OnRotate, OnClose, OnError), управление регистрацией HookRegistry, контекст HookContext и распространённые сценарии расширения."
sidebar_position: 2
---
...
| `HookOnFilter` | Срабатывает только при редукции значения поля (редукция текста сообщения не вызывает хук; хук получает только key поля, не исходное значение) | Запись событий маскирования, аудит |
...
:::warning Клонирование registry
Logger клонирует переданный `registry` при создании (`dd.New(dd.Config{Hooks: registry})` хранит внутри копию), поэтому последующие изменения исходного `registry` не влияют на уже созданный Logger. Для изменения хуков **уже созданного Logger** во время выполнения используйте `logger.AddHook(event, hook)` (внутри Clone-Modify-Store).
:::

## Следующие шаги

- [Аудитные логи](../security/audit-logging) -- интеграция аудита безопасности
- [Распределённая трассировка](../integration/context-tracing) -- интеграция с контекстом
- [Справочник API - Хуки](../../api-reference/core/hooks) -- полный API хуков
