...
description: "Шпаргалка по часто используемому API библиотеки логирования CyberGo DD, охватывающая создание и клонирование логгеров, управление уровнями логирования, конструкторы структурированных полей, конфигурацию ротации и буферизации вывода в файл, правила фильтрации конфиденциальных данных, регистрацию хуков и обработчиков callback, аудитные логи и проверку подписей целостности и другие высокочастотные операции для удобного и быстрого поиска и использования разработчиками."
sidebar_position: 2
---
...
:::tip Нулевые значения конфигурации
Неустановленные поля в литерале `dd.Config{...}` в таблице остаются нулевыми (Level=Debug, IncludeTime/IncludeLevel/DynamicCaller=false, вывод без временной метки/уровня/caller). Для производственной среды рекомендуется брать `dd.DefaultConfig()` за основу и переопределять только нужные поля.
:::
