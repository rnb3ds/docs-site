...
| Условие | Повтор |
|---------|--------|
| Сетевая ошибка (отказ соединения, временные/таймаутные сбои DNS) | Да |
| Ошибка таймаута | Да |
...
// HTTPC будет ждать не более 60 секунд перед повтором (указанные сервером 120s ограничиваются безопасным пределом 60s)

// Сервер возвращает: Retry-After: Fri, 25 Apr 2026 12:00:00 GMT
// HTTPC будет ждать до указанного времени перед повтором (ограничено 60s, если осталось более 60s)
```
...
| Неидемпотентные операции (POST) | Рекомендуется повторять только при сетевых ошибках (по умолчанию также повторяются 5xx/408/429; сузьте через пользовательский RetryPolicy) |
