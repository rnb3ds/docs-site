...
## API предупреждений безопасности
...
Перенаправляет целевой поток вывода предупреждений безопасности (по умолчанию `os.Stderr`); функция определена в `warnings.go`. Передача `io.Discard` полностью подавляет предупреждения. Тестовая среда определяется по имени исполняемого файла (суффикс `.test`) и переменным окружения `GO_TEST`/`GOTEST`.

Две категории опасной конфигурации вызывают предупреждения в **нетестовой среде**, каждая выводится не более одного раза на процесс (`sync.Once`):

| Условие срабатывания | Суть предупреждения |
|---------|---------|
| Клиент, созданный через `TestingConfig()`, используется вне тестовой среды | Сообщает, что проверка TLS-сертификатов, защита от SSRF и валидация URL/Header отключены, и рекомендует `SecureConfig()` или `DefaultConfig()` |
| `Security.InsecureSkipVerify=true` вне тестовой среды | Сообщает, что проверка TLS-сертификатов отключена, использовать её следует только в тестах, а в продакшене — `SecureConfig()` |

```go
// Подавление предупреждений безопасности (только если конфигурация проверена и безопасна)
httpc.SetSecurityWarnOutput(io.Discard)

// Перенаправление в собственный вывод логов
httpc.SetSecurityWarnOutput(log.Writer())
```

:::warning Не злоупотребляйте подавлением
`SetSecurityWarnOutput(io.Discard)` молча поглощает предупреждения безопасности. Используйте её только после полного аудита конфигурации (например, убедившись, что `TestingConfig` применяется только в тестовых бинарниках), и никогда не применяйте для сокрытия предупреждений в продакшене. Полный список практик безопасности см. в [Обзоре безопасности](../../security/).
:::
...
- [Обзор безопасности](../../security/) — SetSecurityWarnOutput и практики безопасной конфигурации
