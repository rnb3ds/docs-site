---
title: Result - HTTPC
description: Справочник API типа ответа Result HTTPC — методы доступа к телу ответа, получения кода состояния, определения статуса, работы с Cookie, парсинга JSON и сохранения файлов.
---

# Result

Result инкапсулирует HTTP-ответ и метаданные запроса, предоставляя удобные методы доступа. Получается через `Client.Request()` или функции на уровне пакета.
...
Возвращает тело ответа в виде исходных байт. Безопасен для nil, возвращает nil.
...
Парсит тело JSON-ответа в целевую переменную. Следует соглашениям `json.Unmarshal`.
...
Возвращает удобочитаемое строковое представление. Чувствительные заголовки автоматически маскируются, тело ответа обрезается до 200 символов.
...
fmt.Println(result.Meta.RedirectCount)  // 1 (выполнено 1 перенаправление)
```
...
Возвращает Result в пул объектов. Данные тела ответа безопасно очищаются (полная очистка для предотвращения остаточных конфиденциальных данных), все внутренние данные обнуляются. После вызова запрещён доступ к любым полям или методам Result.
...
- [Параметры запроса](./options) - Настройка поведения запроса
