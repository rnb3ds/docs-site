...
description: "Конфигурация HTTPC: поля Config и подконфигураций, пять пресетов, правила ValidateConfig и автоповышение повторов при ротации прокси как производное правило."
sidebar_position: 1
---
...
`New()` внутренне выполняет глубокое копирование Config (включая map `Defaults.Headers` и срез `Middleware.Middlewares`) — изменение исходного `cfg` после создания клиента не влияет на его поведение. Два исключения **разделяются по ссылке**: `Retry.CustomPolicy` и `Security.CertificatePinner` не копируются глубоко — реализация pinner по контракту потокобезопасна, но если пользовательская стратегия повторов имеет изменяемое состояние, не передавайте один и тот же экземпляр Config параллельно в несколько `New()`.

:::tip Связанные типы
Для точного управления на уровне запроса используйте функциональные опции `RequestOption` (подробно — в [Параметрах запроса](../core/options)); пользовательская стратегия повторов реализует интерфейс [`RetryPolicy`](../types/interfaces#retrypolicy).
:::
...
`ResponseHeader` по умолчанию равен 0 (отключён), при этом `TimeoutConfig.Request` или `WithTimeout()` служат единственным механизмом таймаута, что обеспечивает полный контроль `WithTimeout()` над длительностью запроса. Этот дизайн подходит для AI API и long-polling — сценариев, требующих увеличенного времени ответа. Устанавливайте положительное значение только при необходимости жёсткого ограничения транспортного уровня (например, для защиты от атак Slowloris), но учтите: этот таймаут действует на уровне Transport, применяется ко **всем запросам, разделяющим один клиент**, и срабатывает раньше, если его значение короче дедлайна, заданного `WithTimeout`.
:::
...
:::tip ProxyRotatePerRequest против ProxyRotateOnStatus
...
#### Соотношение трёх лимитов размера

| Поле | Значение по умолчанию | Описание |
|------|------------------------|----------|
| `MaxResponseBodySize` | 10MB | Ограничивает количество байт исходного тела ответа (при сжатой передаче) |
| `MaxDecompressedBodySize` | 100MB | Ограничивает количество байт тела ответа **после распаковки** (защита от бомб распаковки); если не задано (≤0), откат к `MaxResponseBodySize` |
| `MaxRequestBodySize` | 0 (без ограничения) | Верхний предел тела загружаемого запроса; в отличие от стороны ответа **автоматического отката нет** — действует только при явной установке |
...
`AllowPrivateIPs` по умолчанию `false` — блокирует подключение к приватным/зарезервированным IP (127.0.0.1, 10.x, 192.168.x, 169.254.x и др.; охватывает localhost, loopback, link-local и приватные/зарезервированные диапазоны). Установка `true` **полностью обходит SSRF-проверки диалера на уровне соединения** (не просто разрешает приватные диапазоны) — используйте только при подключении к внутренним сервисам (VPN, прокси, корпоративная сеть); для выборочного разрешения используйте `SSRFExemptCIDRs`. Подробнее см. [Защита от SSRF](../../security/ssrf).
:::
...
Некорректный CIDR заставляет `New()` сразу вернуть ошибку (`invalid configuration: ...`), значение не игнорируется молча.

### Белый список перенаправлений

`RedirectWhitelist` ограничивает перенаправления только перечисленными доменами, по умолчанию `nil` (разрешены все цели перенаправления). Это заслон от «SSRF через перенаправления» — более консервативная альтернатива: `Defaults.FollowRedirects = false` (именно так поступает `SecureConfig()`).
...
Полное описание поведения перенаправлений — в [управлении перенаправлениями](../../guides/redirects).
...
`CustomPolicy` реализует интерфейс [`RetryPolicy`](../types/interfaces#retrypolicy) и может целиком заменить встроенное определение повторяемости; по умолчанию `nil` (используется встроенная стратегия). При настройке ротации прокси по кодам состояния эффективное число повторов автоматически повышается — см. [производные правила конфигурации](#производные-правила-конфигурации-и-внутренние-пересчёты).
...
## Производные правила конфигурации и внутренние пересчёты

При преобразовании публичного Config в конфигурацию движка `New()` откатывает часть нулевых полей к встроенным значениям по умолчанию, а несколько производных правил меняют фактическое поведение:

| Производное правило | Описание |
|---------------------|----------|
| MaxIdleConnsPerHost | Вычисляется как `MaxConnsPerHost / 2`, ограничивается диапазоном [2, 10] и не превышает MaxConnsPerHost; при `MaxConnsPerHost = 0` (без лимита соединений) принимает 10 |
| Откат версий TLS | При `MinTLSVersion` / `MaxTLSVersion` равных 0 откат к TLS 1.2 / TLS 1.3 соответственно |
| Откат MaxRetryDelay | При `Retry.MaxRetryDelay` равном 0 трактуется как 30s |
| TCP KeepAlive | Фиксированно 30s, применяется автоматически вместе с настройками пула соединений — задавать не нужно |
| Автоповышение повторов при ротации прокси | При заданных `ProxyRotateOnStatus` или `ProxyRotatePerRequest` и пуле прокси больше 1 эффективный MaxRetries автоматически повышается до `len(ProxyPool)-1` (верхняя граница 10, понижен не будет) |
| ProxyRotateOnStatus | Передаётся движку как «дополнительные повторяемые коды состояния»: попадание расходует одну единицу бюджета повторов на смену прокси |

:::tip Зачем нужно автоповышение повторов
При стандартном `MaxRetries = 3` пул из 5 прокси после 403 успеет попробовать только 4-й прокси и сдастся. Раз вы настроили ротацию по кодам состояния, намерение — пройти весь пул прокси, поэтому бюджет повторов повышается до «размер пула − 1»; если вы явно задали больший MaxRetries, ваше значение сохраняется без изменений.
:::
...
Сводка правил валидации (при передаче `nil` возвращается `ErrNilConfig`):

| Поле | Ограничение |
|------|-------------|
| `Timeouts.*` (все 5 полей) | 0 – 30min |
| `Connection.MaxIdleConns` | 0 – 1000 |
| `Connection.MaxConnsPerHost` | 0 – 1000 |
| `Connection.ProxyURL` и каждый элемент `ProxyPool` | должен быть корректным URL прокси |
| `Connection.ProxyFailureThreshold` | ≥ 0 |
| `Connection.ProxyCooldown` | 0 – 30min |
| `Connection.ProxyRotateOnStatus` | каждый код состояния в диапазоне 100 – 599 |
...
| `Security.MinTLSVersion` / `MaxTLSVersion` | если оба не 0, то Min ≤ Max |
| `Security.SSRFExemptCIDRs` | каждый элемент должен быть корректным CIDR |
| `Retry.MaxRetries` | 0 – 10 |
| `Retry.Delay` | 0 – 30min |
| `Retry.BackoffFactor` | 1.0 – 10.0 |
| `Retry.MaxRetryDelay` | 0 – 30min |
| `Defaults.MaxRedirects` | 0 – 50 |
| `Defaults.UserAgent` | ≤ 512 символов, без управляющих символов |
| `Defaults.Headers` | каждая пара ключ-значение проходит проверку корректности заголовков |
...
| Поле | Тип | Описание |
|------|-----|----------|
| RequireSecure | `bool` | Требовать у Cookie атрибут Secure |
| RequireHttpOnly | `bool` | Требовать у Cookie атрибут HttpOnly |
| RequireSameSite | `string` | Требуемое значение SameSite, например `"Strict"`, `"Lax"`; пустая строка — без проверки |
| AllowSameSiteNone | `bool` | Разрешать ли SameSite=None; при `false` и пустом `RequireSameSite` Cookie с SameSite=None отклоняются |
| RequireSecureForSameSiteNone | `bool` | При SameSite=None принудительно требовать атрибут Secure (в `DefaultCookieSecurityConfig()` — `true`; при прямой конструкции нулевой структуры — `false`, рекомендуется всегда строить через фабрику) |
...
Сравнение значений полей двух фабрик:

| Поле | DefaultCookieSecurityConfig | StrictCookieSecurityConfig |
|------|------------------------------|----------------------------|
| RequireSecure | `false` | `true` |
| RequireHttpOnly | `false` | `true` |
| RequireSameSite | `""` (не требуется) | `"Strict"` |
