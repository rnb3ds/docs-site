...
description: Справочник API загрузки файлов HTTPC — четыре сигнатуры функций загрузки, конфигурация DownloadConfig, обратные вызовы прогресса, перечисление контрольных сумм и механизм защиты от обхода пути.
---
...
Загрузка с конфигурацией, поддерживает возобновление загрузки и обратный вызов прогресса.
...
:::tip Совет
При установке `Checksum` целостность файла проверяется автоматически после завершения загрузки. При неудачной проверке файл автоматически удаляется и возвращается ошибка — ручное сравнение не требуется.
:::
...
| Защита | Описание |
|------|------|
| Блокировка UNC-путей | Запрещены пути формата `\\server\share` |
| Фильтрация управляющих символов | Запрещены управляющие символы в путях |
| Защита системных путей | Запрещена запись в системные каталоги |
| Обнаружение обхода пути | Обнаружение обхода пути через `../` |
| Обнаружение символических ссылок | Предотвращение атак через символические ссылки |
| Проверка родительских каталогов | Рекурсивная проверка символических ссылок в родительских каталогах |
