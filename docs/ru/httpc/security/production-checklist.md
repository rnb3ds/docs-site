...
description: "Контрольный список безопасности HTTPC для production: проверка TLS, подтверждение AllowPrivateIPs от SSRF и аудит CIDR, настройка таймаутов Timeouts, лимиты размера MaxResponseBodySize, стратегия повторных попыток MaxRetries, освобождение ресурсов и мониторинг аудита AuditMiddleware."
---
...
- [ ] `InsecureSkipVerify` установлено в `false` (значение по умолчанию)
- [ ] `MinTLSVersion` не ниже `tls.VersionTLS12`
- [ ] Не используется `TestingConfig()`

### Защита от SSRF

- [ ] `AllowPrivateIPs` установлено в `false` (значение по умолчанию)
- [ ] При необходимости доступа к внутренним сервисам используйте точное указание через `SSRFExemptCIDRs`
- [ ] При обработке пользовательских URL используйте `SecureConfig()`

### Настройка таймаутов

- [ ] Все значения таймаутов установлены и обоснованы
- [ ] `Timeouts.Request` не равно 0 (предотвращает бесконечное ожидание)
...
- [ ] Для больших ответов используйте потоковую загрузку

### Настройка повторных попыток

- [ ] `MaxRetries` не превышает 5
- [ ] Для неидемпотентных запросов (POST/PUT/PATCH) используйте повторы с осторожностью
- [ ] Включите `EnableJitter` для предотвращения эффекта стадона

### Управление ресурсами

- [ ] Клиент вызывает `Close()` после использования
- [ ] Result вызывает `ReleaseResult()` после использования
...
- [ ] Используйте `RecoveryMiddleware()` для предотвращения падения из-за panic
- [ ] Используйте `LoggingMiddleware()` для логирования запросов
- [ ] Используйте `MetricsMiddleware()` для сбора метрик
- [ ] Для сценариев, чувствительных к безопасности, используйте `AuditMiddleware()`
...
- [ ] Для сценариев, чувствительных к безопасности, включите проверку `CookieSecurity`
- [ ] Используйте `StrictCookieSecurityConfig()` для принуждения к безопасным атрибутам

### Перенаправления

- [ ] Для сценариев с пользовательским вводом URL отключите перенаправления
- [ ] Используйте `RedirectWhitelist` для ограничения целей перенаправлений
