...
description: "Типы данных CyberGo html: Result, ImageInfo, LinkInfo, LinkResource, VideoInfo, AudioInfo, Statistics, BatchResult и поведение MarshalJSON."
sidebar_position: 2
---
...
| Поле | Описание |
|------|------|
| `URL` | Значение атрибута `src` изображения; включает только корректные URL (внутренняя проверка, не экспортируется), `<img>` с некорректным URL не появится в результате |
...
| Поле | Описание |
|------|------|
| `URL` | Значение атрибута `href`; включает только корректные URL (внутренняя проверка, не экспортируется), `<a>` с некорректным URL расходует Position, но не добавляется в срез |
| `Text` | Рекурсивная конкатенация всех текстовых узлов внутри тега `<a>` (включая текст во вложенных тегах) |
...
type LinkResource struct {
    URL   string // Адрес ссылки (при включённом ResolveRelativeURLs уже разрешён в абсолютный на основе BaseURL)
    Title string // Заголовок ссылки
    Type  string // Тип ссылки: "link", "image", "video", "audio", "css", "js", "icon", "media"
}
```

Об HTML-тегах-источниках и управляющих переключателях для каждого значения `Type` см. [Извлечение ссылок](../modules/links#подробное-описание-типов-ссылок).
