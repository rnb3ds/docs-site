...
description: "API ссылок CyberGo html: ExtractAllLinks и GroupLinksByType — скрипты, стили, картинки, группировка по типам, фильтры, сортировка и дедупликация URL."
sidebar_position: 2
---
...
- Логика разрешения централизованно обрабатывается внутри библиотеки (не экспортируется) и **одинаково** относится к контентным ссылкам, изображениям, медиа, тегам source, script, embed и link
...
|------|----------|----------|
| `IncludeContentLinks` | Внутренние ссылки | URL сам по себе является относительным путём, либо (при заданном `BaseURL`) после разрешения находится в том же домене, что и `BaseURL` |
| `IncludeExternalLinks` | Внешние ссылки | URL сам по себе является абсолютным внешним адресом (`http(s)://` или протокольно-относительный `//`), либо (при заданном `BaseURL`) после разрешения оказывается в домене, отличном от `BaseURL` |
