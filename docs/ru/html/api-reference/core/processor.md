...
description: "API Processor CyberGo html: New, Extract, ExtractText, ClearCache, GetStatistics, Close — жизненный цикл, кэш и детектор кодировок для частых вызовов."
sidebar_position: 2
---
...
2. **Установка Scorer**: если настроен пользовательский `Scorer`, он адаптируется к внутреннему интерфейсу (адаптацию выполняет сама библиотека); иначе используется встроенный скорер по умолчанию (внутренняя реализация, не экспортируется; только для чтения, потокобезопасный).
...
- **Кэш**: внутренний кэш имеет собственную блокировку, чтение и запись безопасны. Поведение кэша полностью наблюдается и управляется через открытое API: `Config.MaxCacheEntries = 0` отключает кэш, `ClearCache()` очищает его, `Statistics.CacheHits`/`CacheMisses` отражают количество попаданий.
- **Scorer**: встроенный скорер по умолчанию (внутренняя реализация, не экспортируется) доступен только для чтения. **Пользовательский `Scorer` должен сам обеспечивать потокобезопасность** (например, удерживать внутреннюю блокировку), так как один Processor при параллельных `Extract` вызывает его `Score`/`ShouldRemove` из нескольких goroutine.
:::
...
- **Отключение кэша**: при `MaxCacheEntries = 0` `Extract` пропускает генерацию ключа кэша и чтение/запись кэша (короткое замыкание), не неся никаких накладных расходов на кэш.
...
- **Не используйте после Close**: вызов любого метода после `Close` вернёт `ErrProcessorClosed`. `Close` гарантирует идемпотентность через атомарную операцию CAS (деталь внутренней реализации) — повторный вызов безопасен, но бессмысленен.
:::
