...
description: "Поля Config CyberGo html: 30+ полей в шести группах — ресурсы, безопасность, извлечение, вывод, ссылки; четыре пресета, ограничения и Validate."
sidebar_position: 3
---
...
:::tip Семантика нулевых значений
...
Поэтому **нулевое значение `Config{}` не пройдёт `Validate()` и не может использоваться напрямую** — всегда начинайте с `DefaultConfig()` (или другой предустановки) и при необходимости переопределяйте отдельные поля.
:::
