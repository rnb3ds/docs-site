...
|------|------|--------|------|
| `MaxInputSize` | `int` | `52428800` (50 МБ) | Максимальный размер ввода (байты), диапазон `1`–`52428800` |
| `MaxCacheEntries` | `int` | `2000` | Максимальное количество записей в кэше, `0` отключает кэш (верхний предел `100000`) |
| `CacheTTL` | `time.Duration` | `1ч` | Время жизни записей в кэше, не может быть отрицательным |
| `CacheCleanup` | `time.Duration` | `5м` | Интервал фоновой очистки просроченного кэша, `0` отключает фоновую очистку |
| `WorkerPoolSize` | `int` | `4` | Размер пула воркеров для пакетной обработки, диапазон `1`–`256` |
| `ProcessingTimeout` | `time.Duration` | `30с` | Тайм-аут обработки одного документа, `0` означает без ограничений |
...
| `MaxDepth` | `int` | `500` | Максимальная глубина DOM, диапазон `1`–`500` |
