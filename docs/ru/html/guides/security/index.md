...
description: "Безопасность CyberGo html — три слоя: ввод, обработка, аудит; MaxInputSize и MaxDepth, защита от обхода пути, panic, санитизация HTML, HighSecurityConfig."
sidebar_position: 1
---
...
Для файловых путей также существует **предварительная проверка**: после получения размера через `os.Stat` файл, превышающий лимит, отклоняется до загрузки содержимого в память через `io.ReadAll`, устраняя окно пикового использования памяти по сценарию «сначала полный reads, затем обнаружение превышения».
...
- **Сверхдлинные data URL**: ограничены внутренним лимитом (100 КБ), предотвращая исчерпание памяти большими блоками base64 (этот лимит не настраивается).
...
Возвращённый Windows путь очищается от префикса расширенной длины `\\?\` и обрабатывается через `filepath.Clean`, приводя к формату, согласованному с выводом `filepath.Abs`, что обеспечивает точность последующего сравнения включения.
...
3. **Предварительная проверка размера**: `os.Stat` на проверенном дескрипторе, файлы сверх `MaxInputSize` отклоняются до `io.ReadAll`
