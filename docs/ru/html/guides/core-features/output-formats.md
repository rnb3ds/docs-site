...
| Обработка изображений | Удаление | `![alt](url)` | Список ImageInfo |
...
- alt-текст изображений удаляется; видимый текст ссылок сохраняется (убираются только URL и Markdown-разметка)
- Между заголовками и абзацами сохраняются переносы строк
- Содержимое списков представлено в виде простого текста
- Таблицы рендерятся согласно `TableFormat` (по умолчанию Markdown-таблицы)
...
- [Извлечение ссылок на практике](./link-extraction) - Извлечение ссылок на ресурсы страницы
