...
export default withMermaid(
  defineConfig({
    ...
    themeConfig: {
      ...
      // Native search is intentionally unset. Built-in `local`/`algolia` are
      // both global-index + non-replaceable UI, which conflicts with the
      // per-project scope this site needs (results on a project page must only
      // match that project + the current language). ProjectSearch.vue — injected
      // via theme Layout slots — handles the search box itself, backed by the
      // per-scope JSON indexes generated by scripts/generate-search-index.ts.
