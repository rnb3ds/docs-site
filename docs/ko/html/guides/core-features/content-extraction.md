...
description: "CyberGo html 콘텐츠 추출 실전: 입력 검증, 인코딩 감지, DOM 파싱, 보안 정제, 문서 스마트 인식, 포맷팅을 거치는 전체 추출 흐름과 후보 노드 평가 알고리즘 원리, Result 필드 해석, 커스텀 Scorer 평가기, 자동 인코딩 감지 메커니즘과 모범 사례를 다룹니다."
sidebar_position: 1
---
...
내장 기본 스코어러(내부 구현, 익스포트되지 않음)는 다차원 신호를 종합적으로 평가하여, 가장 높은 점수의 컨테이너를 선택합니다:
