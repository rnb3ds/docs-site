...
description: "CyberGo html 성능 최적화 총람: sync.Pool 로 Processor 재사용, 캐시 적중률 모니터링, 배치 동시성 제어, WorkerPoolSize 튜닝, ProcessingTimeout 타임아웃 설정과 안티패턴 회피로 HTML 처리량을 체계적으로 향상합니다."
sidebar_position: 3
---
...
| > 64 KB | 5점 샘플링 | 헤드 + 테일 + 3개의 균등 분포 샘플링 지점, 총 샘플링 예산 4096 바이트(지점당 약 819 바이트) |
...
| 객체 | 메커니즘 | 역할 |
|------|------|------|
| 풀링된 텍스트 빌더(내부 구현) | `sync.Pool` | 호출 간 underlying `[]byte` 용량 재사용, 매 추출마다 0에서 문서 길이까지 성장하는 것 방지 |
...
1. **사전 정규식 게이팅**: 먼저 내부 미디어 참조 감지(regex 게이팅)로 빠르게 스캔하여, 콘텐츠가 어떤 미디어 참조도 **포함하지 않음**을 확인하면, 모든 정규식 스캔과 iframe/embed/object 속성 추출을 직접 건너뜁니다.
