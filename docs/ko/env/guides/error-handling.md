...
**금지 키 검사(실제로는 `*SecurityError` 반환, `ErrSecurityViolation` 매칭):**
...
// 키 형식 위반(실제로는 *ValidationError, Field=="key" 반환)
var valErr *env.ValidationError
if errors.As(err, &valErr) && valErr.Field == "key" {
    log.Fatalf("유효하지 않은 키 감지: %s", valErr.Message)
}
```
...
- [상수와 오류](/ko/env/api-reference/constants) - 완전한 오류 목록
