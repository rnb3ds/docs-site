...
env 라이브러리는 `.env`, JSON, YAML 세 가지 구성 형식을 지원하며, 형식을 자동으로 감지하고 로드할 수 있습니다.
...
:::tip 권장 사용법
- **코드에서는 플랫 키 이름 사용**: `GetString("DATABASE_HOST")` - 명확하고 효율적
...
config/
├── base.json          # 기본 구성
├── development.env    # 개발 덮어쓰기
├── production.yaml    # 프로덕션 덮어쓰기
...
```text
명령줄 매개변수 > 환경 변수 > local 구성 > 환경 구성 > base 구성
```
...
구성을 지정된 형식으로 직렬화합니다. 먼저 데이터를 준비한 후 대상 형식으로 `env.Marshal`을 호출합니다:
...
:::tip 자동 형식 감지
`env.FormatAuto`를 전달하면 라이브러리가 콘텐츠로 형식을 자동 판단합니다: `data, _ := env.UnmarshalMap(jsonData, env.FormatAuto)`.
:::
...
func main() {
    ...
    err = loader.LoadFiles(
        "config/base.json",       // JSON 기본 구성
        "config/database.yaml",   // YAML 데이터베이스 구성
        "config/app.env",         // .env 애플리케이션 구성
    )
    if err != nil {
        log.Fatal(err)
    }
