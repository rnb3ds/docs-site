...
description: "CyberGo HTTPC는 Go 언어 보안 고성능 HTTP 클라이언트 라이브러리로, TLS 1.2+ 강제 암호화, SSRF 방어 메커니즘, 스마트 지수 백오프 재시도 전략, 양파 모델 미들웨어 체인과 Result 객체 풀 재사용을 내장하여 마이크로서비스 통신과 고동시성 API 호출 시나리오에 적합합니다."
---
...
- **객체 풀 재사용** - 내장 sync.Pool으로 Result 객체 자동 관리, GC 부하 감소
