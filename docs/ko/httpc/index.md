...
description: "CyberGo HTTPC는 Go 언어용 안전하고 고성능 HTTP 클라이언트 라이브러리로, TLS 1.2+ 강제 암호화, SSRF 방어, 지능형 지수 백오프 재시도, 양파 모델 미들웨어 체인, 커넥션 풀 관리, Result 수명주기 자동 관리를 제공하여 마이크로서비스 통신과 고동시성 API 호출 시나리오에 적합합니다."
---
...
- **지능형 재시도** - 지수 백오프 + 지터, 사용자 정의 재시도 전략
- **커넥션 풀 관리** - 고성능 연결 재사용, HTTP/2 지원
...
HTTPC는 세 가지 사용 방식을 제공합니다. 간단한 것부터 유연한 것까지:
...
| `PerformanceConfig()` | 높은 처리량, 큰 커넥션 풀 |
