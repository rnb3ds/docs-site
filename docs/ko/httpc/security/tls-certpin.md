...
description: "HTTPC TLS와 인증서 고정 가이드: TLS 1.2-1.3 버전 제어와 암호 스위트, 커스텀 CA 인증서 로드, mTLS 상호 인증, VerifyPeerCertificate SPKI 공개 키 고정 전략, InsecureSkipVerify 경고와 HTTP/2 협상을 다룹니다."
---
...
가장 일반적인 고정 방식으로, 인증서 체인의任意 인증서의 SPKI 해시를 검증합니다:
...
cfg := httpc.DefaultConfig()
cfg.Security.TLSConfig = &tls.Config{
    InsecureSkipVerify: true, // 표준 검증을 완전히 교체, VerifyPeerCertificate에서 모든 검증을 직접 수행해야 함
    VerifyPeerCertificate: func(rawCerts [][]byte, verifiedChains [][]*x509.Certificate) error {
        // 여기에 인증서 고정 로직 구현
        // 주의: InsecureSkipVerify=true이면 표준 체인 검증이 건너뛰어지므로, 여기서 완전한 인증서 검증을 수행해야 함
        return nil
    },
}
```
