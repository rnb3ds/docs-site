...
description: HTTPC 연결 풀과 프록시 구성 가이드, 연결 풀 매개변수 튜닝, HTTP와 SOCKS5 프록시, DoH 사용자 정의 해석기 및 유휴 연결 관리 전략 상세 설명.
---
...
연결 풀은 HTTP 클라이언트 성능의 핵심 요소입니다. HTTPC는 `ConnectionConfig`로 연결 풀을 관리합니다.
...
| `IdleConn` | 90s | 유휴 연결 타임아웃, 초과 시 종료 |
| `Dial` | 10s | 연결 설정 타임아웃 |
| `TLSHandshake` | 10s | TLS 핸드셰이크 타임아웃 |
| `ResponseHeader` | 30s | 응답 헤더 대기 타임아웃 |

### 시나리오별 추천

| 시나리오 | MaxIdleConns | MaxConnsPerHost | IdleConn |
|------|-------------|-----------------|----------|
| 높은 동시성 API | 100 | 20 | 120s |
| 일반 서비스 | 50 | 10 | 90s |
| 저빈도 요청 | 10 | 2 | 30s |
| 마이크로서비스 내부 | 50 | 10 | 60s |

:::tip 사용 팁
`MaxConnsPerHost`는 활성 연결과 유휴 연결을 모두 포함합니다. 이 한도를 초과하면 새 요청은 연결이 해제될 때까지 대기열에서 대기합니다.
:::
...
:::tip 사용 팁
DoH가 활성화되면 DNS 해석 결과가 `DoHCacheTTL` 시간 동안 캐시됩니다. 모든 DoH 제공자를 사용할 수 없는 경우 시스템 DNS로 폴백합니다.
:::
...
완전한 성능 안티패턴과 최적화 제안은 [성능 최적화](./performance)를 참조하십시오.
