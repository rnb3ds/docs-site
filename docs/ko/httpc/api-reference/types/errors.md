...
description: "HTTPC 오류 타입 API 레퍼런스: ClientError 구조체 8 개 필드와 Code, IsRetryable, Unwrap 등 메서드, ErrorTypeNetwork 등 12 가지 오류 분류 열거, ErrNilConfig 등 센티널 오류 변수와 errors.Is/As 매칭 예제."
sidebar_position: 3
---

# 오류 타입

HTTPC 는 2 계층 오류 모델을 채택합니다: 하위는 표준 `error` 인터페이스, 상위는 분류된 `ClientError` 구조체입니다. 모든 요청 실패 (네트워크 계층) 는 `classifyError`에 의해 컨텍스트를 가진 `ClientError`로 매핑되어, 오류 타입, 재시도 가능 판단, 구조화된 필드를 제공합니다. HTTP 계층 오류 (4xx/5xx) 는 error 로 반환되지 않고 `Result.StatusCode()`로 검사합니다.
...
| `Attempts` | `int` | 시도한 횟수 (첫 요청 포함) | `1`(첫 실패), `4`(3 회 재시도 후) |
...
- URL 과 Method 가 모두 있을 때: `"GET https://example.com: network operation failed: dial tcp ... (attempt 1)"`
- Message 만 있을 때: Message 직접 출력
- Cause 가 있을 때: `": " + Cause.Error()` 추가
- Attempts 가 있을 때 (>0): `" (attempt N)"` 추가

URL 은 출력 전 자동으로 `SanitizeURL`을 거쳐 마스킹됩니다 (자격 증명 제거). 엔진 분류 경로에서 생성된 오류는 이미 마스킹되어 있어 (`urlSanitized=true`), 중복 url.Parse 호출을 건너뛰어 할당을 피합니다.
...
1. **컨텍스트 오류 우선 검사**: Cause 가 `context.Canceled` 또는 `context.DeadlineExceeded`인 경우, 직접 false 반환 (재시도하지 않음)
2. **ErrorType 별 분배**:
...
| `ErrorTypeDNS` | 상황에 따라 | Cause 가 `*net.DNSError`일 때, `IsTemporary` 또는 `IsTimeout`이 true 여야만 재시도 |
| `ErrorTypeHTTP` | 상황에 따라 | StatusCode 가 `retryableStatusCodes`(408/429/500/502/503/504) 에 적중하면 재시도 가능 |
...
:::tip 타임아웃 타입의 미세한 차이
`ErrorTypeTimeout`은 재시도 가능하지만, **컨텍스트 마감으로 트리거된 타임아웃은 재시도할 수 없습니다** — `context.DeadlineExceeded`가 1 단계에서 가로채어지기 (false 반환) 때문입니다. 전송 계층 타임아웃 (예: `net.OpError.Timeout()`) 만이 2 단계에 도달하여 재시도 가능으로 판정됩니다. 이는 사용자가 설정한 `WithTimeout`이 재시도로 돌파되지 않도록 보장합니다.
:::
...
`ClientError.Error()`는 포맷 시 자동으로 `validation.SanitizeURL`을 호출하여 URL 에서 자격 증명 정보를 제거합니다 (`user:pass@host` → `***:***@host`), 민감 정보가 로그와 오류 메시지로 유출되는 것을 방지합니다.
...
엔진 분류 경로 (`classifyErrorWithSanitizedURL`) 는 첫 분류 시 마스킹을 완료하고 `urlSanitized=true`를 설정하며, 이후 `Error()` 호출은 중복 url.Parse 를 건너뛰어 매 로그 출력마다 할당이 발생하지 않습니다.

:::tip 콜백에서의 오류 마스킹
`MetricsMiddleware`와 `LoggingMiddleware` 등 미들웨어의 콜백에서, HTTPC 는 ClientError 가 아닌 타입의 오류 메시지를 추가로 검사하여 원본 URL 을 마스킹된 버전으로 교체하며, 콜백이 자격 증명을 유출하지 않도록 보장합니다.
:::
...
| 변수 | 오류 메시지 | 트리거 조건 |
|------|-------------|-------------|
| `ErrNilConfig` | `"config cannot be nil"` | nil Config 을 `New`/`ValidateConfig`에 전달 |
| `ErrInvalidTimeout` | `"invalid timeout"` | 타임아웃 값이 음수이거나 30 분 상한 초과 |
| `ErrInvalidRetry` | `"invalid retry configuration"` | MaxRetries 가 0–10 외, BackoffFactor 가 1.0–10.0 외 |
| `ErrInvalidConnection` | `"invalid connection configuration"` | MaxIdleConns/MaxConnsPerHost 범위 초과, ProxyURL 형식 오류 |
| `ErrInvalidSecurity` | `"invalid security configuration"` | MaxResponseBodySize 가 0–1GB 범위 초과 |
| `ErrInvalidMiddleware` | `"invalid middleware configuration"` | MaxRedirects 가 0–50 외, UserAgent 가 너무 길거나 제어 문자 포함 |
...
| 변수 | 오류 메시지 | 트리거 조건 |
|------|-------------|-------------|
| `ErrEmptyFilePath` | `"file path cannot be empty"` | DownloadConfig.FilePath 가 비어 있음 |
| `ErrFileExists` | `"file already exists"` | 파일이 이미 존재하고 Overwrite=false 이며 ResumeDownload=false |
...
보안 경고의 출력 대상을 변경합니다 (기본 `os.Stderr`). `warnings.go`에 정의되어 있습니다. `io.Discard`를 전달하면 경고를 완전히 억제할 수 있습니다. 테스트 환경은 실행 파일 이름 (`.test` 접미사) 과 `GO_TEST`/`GOTEST` 환경 변수로 판별합니다.

두 종류의 위험한 구성이 **테스트 이외 환경**에서 경고를 발생시키며, 각 경고는 프로세스당 최대 한 번만 출력됩니다 (`sync.Once`).

| 트리거 조건 | 경고 요점 |
|---------|---------|
| `TestingConfig()`로 생성한 클라이언트를 테스트 이외 환경에서 사용 | TLS 인증서 검증, SSRF 방호, URL/Header 검증이 모두 비활성화되었음을 알리고 `SecureConfig()` 또는 `DefaultConfig()` 사용을 권장 |
| `Security.InsecureSkipVerify=true`이고 테스트 이외 환경 | TLS 인증서 검증이 비활성화되었음을 알리며, 테스트에서만 사용해야 하고 프로덕션에서는 `SecureConfig()`를 사용해야 함을 안내 |

```go
// 보안 경고 억제 (구성이 안전하다고 확인된 경우에만 사용)
...
:::warning 억제 남용 주의
`SetSecurityWarnOutput(io.Discard)`는 보안 경고를 조용히 삼켜 버립니다. 구성을 충분히 감사했을 때 (예: `TestingConfig`가 테스트 바이너리에서만 사용되는지 확인) 만 사용하고, 프로덕션 배포에서 경고를 가리는 용도로 사용하지 마세요. 보안 실천의 전체 목록은 [보안 개요](../../security/)를 참조하세요.
:::

## 실용적 매칭 패턴

### errors.As 로 ClientError 추출
...
### errors.Unwrap 으로 오류 체인 순회

```go
var clientErr *httpc.ClientError
if errors.As(err, &clientErr) {
    // Cause 는 하위 오류 (예: *net.OpError)
...
:::tip 세 가지 매칭 방식의 선택
- `errors.As`: ClientError 의 구조화된 필드 (Type/Code/URL/Attempts 등) 에 접근 필요할 때
...
- [보안 개요](../../security/) - SetSecurityWarnOutput 과 보안 구성 실천
