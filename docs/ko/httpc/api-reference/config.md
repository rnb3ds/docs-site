---
title: 설정 - HTTPC
description: HTTPC 설정 시스템 API 참조. Config 메인 구조체와 다섯 가지 하위 설정 그룹의 모든 필드 설명, 다섯 가지 사전 설정 함수 및 Validate 검증 메서드를 다룹니다.
---
...
메인 설정 구조체입니다. `DefaultConfig()`를 통해 안전한 기본값을 얻을 수 있습니다.
...
안전한 기본 설정입니다. SSRF 방어가 기본적으로 활성화되어 있습니다.
...
보안 우선 설정입니다. 더 짧은 타임아웃, 자동 리다이렉트 비활성화, 엄격한 SSRF 방어를 적용합니다.
...
고처리량 설정입니다. 더 큰 연결 풀과 긴 타임아웃을 적용하며, 보안 검증은 유지합니다.
...
테스트 환경 설정입니다. 보안 검사를 비활성화하고 짧은 타임아웃을 적용합니다.
...
경량 설정입니다. 재시도와 리다이렉트를 비활성화하고 최소 연결 풀을 적용합니다.
...
설정의 유효성을 검증합니다. `New()` 내부에서 자동으로 호출되며, 명시적으로 호출할 수도 있습니다.
...
기본 Cookie 보안 설정입니다. Secure/HttpOnly/SameSite 속성을 요구하지 않지만, SameSite=None인 Cookie는 반드시 Secure 속성을 설정하도록 강제합니다.
...
엄격한 Cookie 보안 설정입니다. Secure, HttpOnly 및 SameSite=Strict을 요구합니다.
