...
토큰 버킷 알고리즘을 채택: 토큰은 경과 시간에 비례하여 **비례 보충**되며 (고정 윈도우 재설정이 아님), 보충 후 잔여 시간을 보존하여 속도를 균일하게 유지; 매 요청마다 토큰을 소비하며, 토큰이 부족하면 거부합니다.

내부적으로 최대 10000개의 속도 제한 키(`maxBuckets`)를 추적합니다. 상한 도달 시 먼저 만료 버킷(2배 윈도우 이상 미활동)을 축출하고, 여전히 가득 차면 가장 오래된 약 10%를 배치 축출하여 O(n) 스캔 비용을 분할 상환합니다.
