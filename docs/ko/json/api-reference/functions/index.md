...
json 패키지가 제공하는 최상위 함수로, Processor 인스턴스를 생성하지 않고 직접 호출할 수 있습니다. 기능별 분류는 다음과 같습니다:

## [조회 및 가져오기](./query)

경로 쿼리, 타입 안전 조회, 안전 조회, 배치 조회 함수.
...
JSON 을 대상 객체로 파싱, Processor 인스턴스 파싱, JSON 유효성/Schema 검증 함수.
...
JSON 배열, 객체, 중첩 구조, 파일을 순회하는 반복 함수.
...
파일 스트리밍 반복 시나리오 가이드와 실습 (패키지 레벨 `ForeachFile*` 함수의 API 레퍼런스는 [반복 메서드](./iterate#파일-반복-함수) 참조).
...
타입 변환, 비교, 캐시 관리, 오류 처리 등 유틸리티 함수.
...
| 용도 | 추천 함수 | 문서 |
|------|----------|------|
| 단일 값 조회 | `GetString`, `GetInt`, `GetFloat`, `GetBool` | [조회 및 가져오기](./query#경로-쿼리-함수) |
| 임의 타입 조회 | `Get`, `GetTyped[T]` | [조회 및 가져오기](./query#제네릭-조회-함수) |
| 기본값으로 조회 | `GetString(data, path, "default")` | [조회 및 가져오기](./query#타입-안전-조회-함수) |
| 제네릭 조회 | `GetTyped[T](data, path, defaultValue...)` | [조회 및 가져오기](./query#제네릭-조회-함수) |
| 배치 조회 | `GetMultiple` | [조회 및 가져오기](./query#processor-확장-메서드) |
| JSON 수정 | `Set`, `SetCreate` | [수정 작업](./modify) |
...
| 출력 인쇄 | `Encode` + `fmt.Println`, `EncodePretty` | [출력 포맷팅](../../getting-started/print) |
...
| 캐시 예열 | `WarmupCache` | [배치 작업](./batch#캐시-예열-warmupcache) |
| 검증 | `Valid` | [파싱 및 검증](./parse#검증-함수) |
| JSON Schema 검증 | `ValidateSchema` | [파싱 및 검증](./parse#validateschema) |
| Schema 기본 설정 | `DefaultSchema`, `DefaultSchemaConfig` | [Schema 검증](../schema#defaultschema) |
| 파일 읽기/쓰기 | `LoadFromFile`, `SaveToFile` | [파일 I/O](./file-io#파일-읽기와-쓰기) |
| 파일 읽기/쓰기 (구조체) | `MarshalToFile`, `UnmarshalFromFile` | [파일 I/O](./file-io#직렬화-편의-메서드) |
| 스트리밍 I/O | `LoadFromReader`, `SaveToWriter` | [파일 I/O](./file-io#스트리밍-i-o) |
...
| JSONL 스트리밍 처리 | `StreamJSONL`, `StreamLinesInto[T]`, `FirstJSONL` | [JSONL](./jsonl#jsonl-스트리밍-처리-함수-패키지-레벨) |
...
- [Config](../config) - 설정 옵션
- [상수 및 오류](../constants) - 오류 타입
- [인터페이스 정의](../interfaces) - 확장 인터페이스
- [경로 표현식 문법](../../getting-started/path-syntax) - 경로 문법 상세
