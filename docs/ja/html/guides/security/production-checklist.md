...
description: "CyberGo html 本番デプロイ向けセキュリティチェックリスト：P0/P1/P2 優先度別チェックで、HighSecurityConfig プリセット、Processor ライフサイクル管理、監査監視、context タイムアウト制御、エラー処理とリソース解放などリリース前の必須項目を網羅します。"
sidebar_position: 3
---
...
- ライブラリ内部はすでに `io.ReadAll` でメモリに読み込む前に `os.Stat` でファイルサイズを事前チェックし、上限超過のファイルを拒否しています。「読み切ってから上限超過に気づく」というメモリピークの窓は閉じられています。外層の業務でさらに一道のサイズ事前チェックを加えるのは深度防御にあたります。
