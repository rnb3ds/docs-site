...
description: "CyberGo html パフォーマンス最適化総覧：sync.Pool による Processor 再利用、キャッシュ命中率監視、バッチ並行制御、WorkerPoolSize チューニング、ProcessingTimeout タイムアウト設定とアンチパターンの回避で、HTML 処理のスループットを体系的に向上させます。"
sidebar_position: 3
---
...
| > 64 KB | 5 点サンプリング | 先頭 + 末尾 + 3 つの均等分布サンプリング点、総サンプリング予算 4096 バイト（各点約 819 バイト） |
...
| オブジェクト | メカニズム | 作用 |
|------|------|------|
| プール化テキストビルダー（内部実装） | `sync.Pool` | 呼び出し間で基盤の `[]byte` 容量を再利用、毎回の抽出でゼロからドキュメント長まで成長するのを回避 |
...
1. **前置正規表現ゲート**：まず内部のメディア参照検出（regex ゲート）で高速スキャンし、コンテンツにメディア参照が**含まれない**ことを確認した場合、すべての正規表現スキャンと iframe/embed/object 属性抽出をスキップします。
