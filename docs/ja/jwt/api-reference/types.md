...
トークンバケットアルゴリズムを採用：トークンは経過時間に応じて**比例補充**され（固定ウィンドウのリセットではない）、補充後に残余時間を保持することでレートの均一化を保証します；各リクエストでトークンを消費し、トークン不足の場合は拒否します。

内部では最大 10000 個のレート制限キー（`maxBuckets`）を追跡します。上限に達した場合、まず期限切れバケット（2 倍ウィンドウ時間を超えて非アクティブなもの）をエビクションし、それでも満杯の場合は最も古い約 10% を一括エビクションし、O(n) のスキャンコストを償却します。
