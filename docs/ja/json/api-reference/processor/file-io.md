...
description: "CyberGo JSON Processor ファイルメソッド：LoadFromFile/LoadFromReader ロード、SaveToFile/MarshalToFile 保存、UnmarshalFromFile 読み取り、SaveToWriter ストリーム書き込み。"
sidebar_position: 9
---

# ファイル操作メソッド

Processor は JSON ファイルの読み書きとストリーミングロードのメソッドを提供し、ファイル、`io.Reader`、`io.Writer` の 3 種類のデータソースをカバーします。ファイル系メソッドは操作前にセキュリティパス検証を行います（[関数リファレンス](../functions/file-io#セキュリティ-ファイルパス検証)を参照）。
...
ファイルから JSON データをロードし、**生の文字列**を返します（ファイルのバイト順序と空白を保持し、再エンコードしません）。読み取りバイト数は `MaxJSONSize` の制限を受けます。
...
### LoadFromFileAsData（非公開化済み）

::: warning API 変更の説明
LoadFromFileAsData は内部メソッド（`loadFromFileAsData`）に変更され、公開 API としてはエクスポートされなくなりました。`LoadFromFile` + `Parse` の組み合わせを代わりに使用してください：
...
`io.Reader` から JSON データをロードし、生の文字列を返します。読み取りは `MaxJSONSize` の制限を受け、`os.File`、HTTP Body、パイプなどのストリーミングソースに適しています。
...
### LoadFromReaderAsData（非公開化済み）

::: warning API 変更の説明
LoadFromReaderAsData は内部メソッド（`loadFromReaderAsData`）に変更され、公開 API としてはエクスポートされなくなりました。`LoadFromReader` + `Parse` の組み合わせを代わりに使用してください：
...
データを JSON ファイルとして保存します。親ディレクトリを自動作成し、**アトミック書き込み**（一時ファイル + rename）を採用します。文字列 / `[]byte` 入力は事前解析され、二重エスケープを回避します。
...
データを JSON にエンコードしてファイルに書き込みます。親ディレクトリを自動作成し、アトミック書き込みを行います。`SaveToFile` と**同じ「エンコード + アトミック書き込み」パイプラインを共用**します：同様に文字列 / `[]byte` 入力を事前解析して二重エスケープを回避し、渡された `cfg` を完全に尊重します（`Pretty` / `Indent` / `EscapeHTML` などすべて有効）——両者の差は現在ではエラーメッセージの操作名のみで、過去の動作差は解消されています。
...
ファイルから JSON を読み取ってターゲット変数にデコードします。読み取りは `MaxJSONSize` の制限を受けます。
...
データを JSON にエンコードして `io.Writer` に書き込みます。文字列 / `[]byte` 入力は同様に事前解析され二重エスケープを回避します。ファイルパスを扱わないため、パス検証は行いません。
...
Processor は `ForeachFile` 系メソッドを提供し、ファイルから直接 JSON コレクションを反復します。`LoadFromFile` + `Foreach` の便利な組み合わせです：

| メソッド | 用途 |
|------|------|
| `ForeachFile(path, fn, cfg...)` | ファイルのルートレベル配列 / オブジェクトを反復 |
| `ForeachFileWithPath(path, pathExpr, fn, cfg...)` | ファイル内の指定パス配下のコレクションを反復 |
| `ForeachFileChunked(path, chunkSize, fn, cfg...)` | 大型配列をバッチ単位で反復 |
...
コールバックは `item.Break()` による早期終了に対応しています。ストリーミング処理と大規模ファイル最適化の詳細は[ストリーミング処理](../../streaming/large-files)を参照してください。

## メソッド選択

| シナリオ | 推奨メソッド |
|------|----------|
| 生の文字列が必要 | `LoadFromFile` / `LoadFromReader` |
| 解析済みデータが必要 | `LoadFromFile` + `Parse` / `LoadFromReader` + `Parse` |
| Go 値をファイルに保存 | `SaveToFile` / `MarshalToFile` |
| ファイルから読み取って構造体にデコード | `UnmarshalFromFile` |
| Writer に書き込み | `SaveToWriter` |
| ファイル内のコレクションを反復 | `ForeachFile` 系 |

## 関連

- [解析と検証](./parse) - Parse/Valid 解析メソッド
- [ファイル関数](../functions/file-io) - パッケージレベルのファイル読み書き関数（パスセキュリティ検証の詳解を含む）
