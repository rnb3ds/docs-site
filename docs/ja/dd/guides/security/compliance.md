...
`HealthcareConfig()` は `DefaultSecureConfig()`（完全パターンセット）の上に、以下を追加フィルタリング：
...
`FinancialConfig()` は `DefaultSecureConfig()`（完全パターンセット）の上に、以下を追加フィルタリング：
...
`GovernmentConfig()` は `DefaultSecureConfig()`（完全パターンセット）の上に、以下を追加フィルタリング：
...
- [機密データフィルタリング](./sensitive-filtering) -- フィルタリング機能詳解
